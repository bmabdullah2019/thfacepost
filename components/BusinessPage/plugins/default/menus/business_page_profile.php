...
$ossnmenu->sortMenu('business_page_profile');
$params['menu'] = $Ossn->menu['business_page_profile'];
...
foreach($params['menu'] as $menu) {
		...
		if($menu_width != 1 && $i <= $menu_width) {
				foreach($menu as $name => $link) {
						$class = "menu-businesspage-profile-" . $link['name'];
						if(isset($link['class'])) {
				...
				foreach($menu as $name => $link) {
						$class = "dropdown-item menu-businesspage-profile-" . $link['name'];
						if(isset($link['class'])) {
								$link['class'] = $class . ' ' . $link['class'];
						} else {
								$link['class'] = $class;
