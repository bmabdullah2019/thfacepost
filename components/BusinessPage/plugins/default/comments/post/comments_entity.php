...
$business = false;
if(isset($params['params']['businesspage']) && $params['params']['businesspage']){
		$business = $params['params']['businesspage'];
}
...
if ($comments) {
    foreach ($comments as $comment) {
			...
			if($business && $comment->owner_guid == $business->owner_guid){
...
if (ossn_isLoggedIn() && $allow_post_comment){	
	...
	if($business && ($business && $business->owner_guid == ossn_loggedin_user()->guid)){
